...
## Collect node and edge data
    ...
    all_node_trans_GSEA_KEGG <- rbind(emap_to_network_df(emap_Trans.5448.GSEA.KEGG.sig, what = "vertices") %>% mutate(condition = "5448"),
                                     emap_to_network_df(emap_Trans.HKU419.GSEA.KEGG.sig, what = "vertices") %>% mutate(condition = "HKU419"),
                                     emap_to_network_df(emap_Trans.PS003.GSEA.KEGG.sig, what = "vertices") %>% mutate(condition = "PS003"),
                                     emap_to_network_df(emap_Trans.PS006.GSEA.KEGG.sig, what = "vertices") %>% mutate(condition = "PS006"),
                                     emap_to_network_df(emap_Trans.SP444.GSEA.KEGG.sig, what = "vertices") %>% mutate(condition = "SP444")) %>%
      dplyr::group_by(name) %>%
    ...
    ##   <chr>                           <chr>                         <int>      <dbl>
    ## 1 Fructose and mannose metabolism Amino sugar…                      2      0.278
    ## 2 Galactose metabolism            Amino sugar…                      1      0.261
    ## 3 Glycolysis / Gluconeogenesis    Amino sugar…                      1      0.2  
    ## 4 Phosphotransferase system (PTS) Amino sugar…                      2      0.288
    ## 5 Starch and sucrose metabolism   Amino sugar…                      2      0.211

    head(all_node_trans_GSEA_KEGG, 5)

    ## # A tibble: 5 × 5
    ##   name              nb_condition_with_node condition_info gene_set_size p.adjust
    ##   <chr>                              <int> <chr>                  <dbl> <chr>   
    ## 1 Amino sugar and …                      2 PS003,SP444             18.5 0.00750…
    ## 2 Aminoacyl-tRNA b…                      1 SP444                   15   0.04721…
    ## 3 Ascorbate and al…                      5 5448,HKU419,P…          17.8 0.00980…
    ## 4 Biosynthesis of …                      5 5448,HKU419,P…          32.8 0.02125…
    ## 5 Biosynthesis of …                      2 HKU419,SP444            52.5 0.00968…

    # 3. Save data
    write.table(all_edge_trans_GSEA_KEGG, "/Users/wmujchariyak/Desktop/Strep_all_edge_trans_GSEA_KEGG_new.tsv", sep = "\t", row.names = F)
    write.table(all_node_trans_GSEA_KEGG, "/Users/wmujchariyak/Desktop/Strep_all_node_trans_GSEA_KEGG_new.tsv", sep = "\t", row.names = F)
